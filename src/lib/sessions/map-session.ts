...
const readObject = (value: unknown): SessionLike =>
  value && typeof value === 'object' && !Array.isArray(value) ? value as SessionLike : {}
...
const readFirstValue = (session: SessionLike, keys: string[]) => {
  for (const key of keys) {
    if (session[key] !== undefined && session[key] !== null && session[key] !== '') return session[key]
  }
...
const readPerson = (session: SessionLike, key: 'client' | 'trainer') => {
  ...
  const timezone = readString(readNestedFirstValue(session, ['timezone', 'time_zone', 'timeZone']), '')
  const timezoneRegion = timezone ? formatTimezoneRegion(timezone) : ''
  const name =
    readString(person.name, '') ||
    readString(person.full_name, '') ||
    readString(session[`${key}_name`], key === 'client' ? 'Unknown Client' : 'Unknown Trainer')

  return {
    name,
    avatar: readString(person.avatar, '') || readString(person.avatar_url, '') || readString(person.display_picture, '') || undefined,
    country: readString(person.country, readString(session[`${key}_country`], timezoneRegion || 'N/A')),
  }
}
...
const formatScheduled = (session: SessionLike) => {
  ...
  const date = readNestedFirstValue(session, ['scheduled_date', 'scheduledDate', 'session_date', 'sessionDate', 'date'])
  const time = readNestedFirstValue(session, ['scheduled_time', 'scheduledTime', 'session_time', 'sessionTime', 'time'])

  if (typeof date === 'string' && date.trim() && typeof time === 'string' && time.trim()) {
    return formatDateTime(`${date}T${time}`)
  }

  return formatDateTime(readNestedFirstValue(session, ['created_at', 'createdAt']))
}
...
const mapState = (value: unknown): Session['state'] => {
  ...
  if (['unconfirmed', 'pending_confirmation'].includes(normalized)) return 'Unconfirmed'
  return 'Scheduled'
}
...
export const mapBackendSessionToSession = (backendSession: unknown): Session => {
  ...
  return {
    ...
    type: mapType(session.type ?? session.session_type ?? session.subscription_type),
    scheduled: formatScheduled(session),
    duration: formatDuration(session),
    amount: readNumber(session.amount),
    clientConf: mapConfirmation(session.clientConf ?? session.client_confirmed ?? session.client_confirmation ?? session.client_joined),
    trainerConf: mapConfirmation(session.trainerConf ?? session.trainer_confirmed ?? session.trainer_confirmation ?? session.trainer_joined),
...
export const mapBackendSessionsResponse = (payload: unknown): Session[] => {
  ...
  return possibleList.map(mapBackendSessionToSession).filter((session) => session.id)
}
