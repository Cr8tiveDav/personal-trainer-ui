...
function getInitialState(
  ...
  const firstNew = WEEK_DAYS.find(
    (d) => daySchedules[d.value].enabled && !existingSlots.some((s) => s.day_of_week === d.value),
  )
...
export function AvailabilitySetupPanel({
  ...
  const [daySchedules, setDaySchedules] = useState<DaySchedules>(initial.daySchedules)
  const [selectedDay, setSelectedDay] = useState<number | null>(initial.selectedDay)
  ...
  function updateSelectedDayTime(field: 'startTime' | 'endTime', value: string) {
  ...
  return (
    <div className='flex flex-col lg:flex-row gap-0 bg-white rounded-xl border border-gray-100 overflow-hidden'>
        ...
        <div className='flex flex-wrap gap-2 mb-6'>
          {WEEK_DAYS.map((day) => {
            ...
            return (
              ...
              >
                <span className='text-sm font-medium leading-none'>{day.label}</span>
                <span
                  className={cn(
                    'text-xs mt-1.5',
                    isPersisted
      ...
      <div className='w-full lg:w-[280px] shrink-0 p-6 bg-white flex flex-col gap-8 border-t lg:border-t-0 border-gray-100'>
        <div>
          <div className='flex items-start justify-between gap-3'>
            <div>
              ...
              <p className='text-xs text-gray-500 mt-1'>Toggle off to mark unavailable.</p>
            </div>
            <button
              type='button'
              role='switch'
        ...
        <div>
          <h3 className='text-sm font-semibold text-gray-900 mb-2'>Time zone</h3>
          {editingTimezone ? (
