  ...
  return (
    <>
        ...
        {isMobile ? (
          ...
          >
            {collapsed ? (
              ...
              <img src='/collapse.svg' alt='collapse' width={20} height={20} className='h-4 w-4' />
            )}
          </button>
        )}
      </div>
      ...
      <nav className='flex flex-1 flex-col gap-1 overflow-y-auto'>
        {NAV_ITEMS.map((item) => (
          <TrainerSidebarItem key={item.href} {...item} collapsed={collapsed} />
        ))}
      </nav>
      ...
      <div className={cn('flex items-center gap-3 rounded-lg border border-gray-100 p-3', collapsed && 'justify-center')}>
        {userAvatar ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={userAvatar} alt={userName} width={36} height={36} className='rounded-full object-cover shrink-0 h-9 w-9' />
        ) : (
...
export function TrainerSidebar({ userName, userEmail, userAvatar, mobileOpen, onMobileClose }: TrainerSidebarProps) {
